...
def show_data(new_year, pre_year, data, s_pre_single,s_pre_double, s_pre_triple):
    ...
    plt.plot(year[1:], s_pre_double[2:81],
             color='red', label="二次平滑预测值")
    plt.plot(year[1:], s_pre_triple[2:81],
...
def main():
    ...
    print(output_s)
    sns.distplot(s_pre_double[1:81]-number)
    result1 = check_anomaly_by_d(s_pre_single[1:81]-number)
    result2 = check_anomaly_by_d(s_pre_double[1:81] - number)
    result3 = check_anomaly_by_d(s_pre_triple[1:81] - number)
