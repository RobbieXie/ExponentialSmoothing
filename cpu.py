...
def show_data(new_year, pre_year, data, s_pre_single,s_pre_double, s_pre_triple):
    ...
    plt.plot(year[1:], s_pre_double[2:41],
             color='red', label="二次平滑预测值")
    plt.plot(year[1:], s_pre_triple[2:41],
...
def main():
    ...
    print(output_s)
    sns.distplot(s_pre_double[1:41]-number)
    result = check_anomaly_by_d(s_pre_double[1:41]-number)
    print(result)
